...
(let [polygon {}]
  ...
  (fn polygon.new [x y c] 
    { :module polygon :color c :points [[x y]] })
  (fn polygon.empty [c]
    { :module polygon :color c :points [] })
  (fn polygon.draw-bg [shape color-mode] 
    (let [pt-count (length shape.points)]
      ...
      (if 
        (> pt-count 1)
        (when (> pt-count 2)
          (let [(ok ret) (pcall love.math.triangulate (v.flatten shape.points))]
            (when ok
              (each [i tri (ipairs ret)] 
  ...
  (fn polygon.draw-selected [shape server-mode slide-offset]
    (let [pt-count (length shape.points)
          points shape.points
